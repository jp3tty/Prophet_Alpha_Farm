...
class PortfolioAnalyzer:
    ...
    def simulate_portfolios(self, returns, num_portfolios=20000):
        """
...
def main():
    ...
    simulation_results, optimal_portfolios = analyzer.analyze(num_simulations=1000)
    
    # Print formatted portfolio allocations
    max_sharpe_portfolio = optimal_portfolios.iloc[0]
    min_vol_portfolio = optimal_portfolios.iloc[1]
    
    analyzer.print_portfolio_allocation(max_sharpe_portfolio, "Maximum Sharpe Ratio")
    analyzer.print_portfolio_allocation(min_vol_portfolio, "Minimum Volatility")
    
    # Print key metrics
    print("\nPortfolio Metrics:")
    print("-" * 40)
    print(f"{'Metric':<15} {'Max Sharpe':<15} {'Min Vol':<15}")
    print("-" * 40)
    print(f"{'Return':<15} {max_sharpe_portfolio['Return']*100:>8.2f}% {min_vol_portfolio['Return']*100:>14.2f}%")
    print(f"{'Volatility':<15} {max_sharpe_portfolio['Volatility']*100:>8.2f}% {min_vol_portfolio['Volatility']*100:>14.2f}%")
    print(f"{'Sharpe Ratio':<15} {max_sharpe_portfolio['Sharpe']:>8.2f} {min_vol_portfolio['Sharpe']:>14.2f}")
