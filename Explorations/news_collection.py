...
def get_stock_news(symbol, num_articles=100):  # Changed default to 100
    """
    Fetch recent news articles for a given stock symbol using Finviz.
    
    Args:
        symbol (str): Stock symbol (e.g., 'AAPL', 'GOOGL')
        num_articles (int): Number of articles to retrieve (default: 100)
    ...
    try:
        ...
        response = requests.get(url, headers=headers, timeout=15)  # Increased timeout for larger request
        ...
        # Process all found rows up to num_articles
...
def main():
    print(f"Starting news collection for {len(TICKER_SYMBOLS)} symbols (up to 100 articles each)...")
    successful_saves = 0
    ...
    for symbol in TICKER_SYMBOLS:
        ...
        # Add a longer delay between requests due to larger data collection
        time.sleep(3)  # Increased delay to be more considerate of the server
